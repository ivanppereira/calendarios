...
export function validarCalendario(dias, tipoCalendario, atividades = []) {
  ...
  // Identificar se há períodos com início e fim definidos
  const temPeriodosDefinidos = cfg.nomesPeriodo.some((_, i) => marcos[`inicio_periodo_${i}`] && marcos[`fim_periodo_${i}`]);

  for (const [key, info] of Object.entries(dias)) {
    if (info.tipo === "ferias") feriasTotal += 1;

    // Se houver períodos definidos, verificar se o dia está dentro de pelo menos um período
    if (temPeriodosDefinidos) {
      const dentroDeAlgumPeriodo = cfg.nomesPeriodo.some((_, i) => {
        const ini = marcos[`inicio_periodo_${i}`];
        const fim = marcos[`fim_periodo_${i}`];
        return ini && fim && key >= ini && key <= fim;
      });
      if (!dentroDeAlgumPeriodo) continue;
    }
